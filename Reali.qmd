...
computing average grade per student per course
...
avg_two <-
  avg_per_course %>%
  inner_join(course_ids, by = "course_id") %>%
  left_join(students %>% select(id, group), by = "id") %>%
  summarise(avg = mean(mean_grade), .by = c(group, course_name))

ggplot(avg_two, aes(x = group, y = avg, fill = course_name)) +
